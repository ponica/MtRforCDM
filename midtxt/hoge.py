...
def tcgetpianoroll(midi):
    op = []
    for t in range(len(midi_data.instruments)):
        data = midi_data.instruments[t].get_piano_roll(50)
...
def getpianoroll(midi):
    ...
    for t in range(len(midi_data.instruments)):
        if (midi_data.instruments[t].is_drum==True):
            print "skipped drumtrack"
            continue
        data = midi_data.instruments[t].get_piano_roll(50)
        ooct = oneoround(data)
        ooct = ooct.T
...
def writetxt(path,data):
    path = path.lstrip('./')
    tgtdir = "./output.combine/1oct/"+os.path.split(path)[0]
    if not(os.path.isdir(tgtdir)):
        os.makedirs(tgtdir.lstrip('./'))
    with open('./output.combine/1oct/'+path.rstrip('.mid')+'.txt', 'wt') as f:
...
def tcwritetxt(path,data):
    path = path.lstrip('./')
    tgtdir = "./output.combine/tcr/"+os.path.split(path)[0]
    if not(os.path.isdir(tgtdir)):
        os.makedirs(tgtdir.lstrip('./'))
    with open('./output.combine/tcr/'+path.rstrip('.mid')+'.txt', 'wt') as f:
        writer = csv.writer(f,lineterminator='\n')
        writer.writerows([data])
