...
def conv(dataset):
    ...
    for file in infiles:
        ...
        """convert csv"""
        red = int(0.01*framerate)
        window = int(0.025*framerate)
        fr = 0
        oa=[]
        ...
        for fr in range(mfr/red):
            X = np.fft.fft(data[fr:fr+window])
            amp = [np.sqrt(c.real ** 2 + c.imag ** 2) for c in X]
            freqList = np.fft.fftfreq(red, d=1.0/framerate)
            fq = 0
            fqList = 0.0
            fqp = []
            while (ifq*(2**(fqList/12))) < 1760:
                tgt = ifq*(2**(fqList/12))
                fpow = int( amp[ getNearestValue(freqList, tgt) ] )//20000
                fqp.append(fpow)
                fqList = fqList+1
            oa.append(fqp)
            fr = fr+red
